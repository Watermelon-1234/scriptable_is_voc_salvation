...
async function main() {
  ...
  // Step 1: 如果需要更新，每日單字與未出題池都刷新
  if (needUpdateDaily()) {
    let newWords = await fetchDailyWords();
    saveJSON(dailyPath, newWords);   // 完整池
    saveJSON(remainPath, newWords);  // 未出題池
    console.log("已更新每日單字");
  }

  let dailyWords = loadJSON(dailyPath);  // 完整 500 題
  let remainWords = loadJSON(remainPath); // 剩餘待問
  let wrongWords = loadJSON(wrongPath);
  ...
  // Script.setShortcutOutput(JSON.stringify(data));
...
const GITHUB_TOKEN = "YOUR_GITHUB_PAT"; // 替換成你的 PAT
const REPO = "Watermelon-1234/scriptable_is_voc_salvation";
const BRANCH = "voc";
